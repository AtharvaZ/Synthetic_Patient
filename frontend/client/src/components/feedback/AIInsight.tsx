...
export default function AIInsight({ insight }: AIInsightProps) {
  ...
  return (
    <div className={`rounded-2xl p-6 border ${isDarkMode ? "bg-gradient-to-br from-[#137fec]/25 to-teal-500/25 border-primary/30" : "bg-gradient-to-br from-[#137fec]/20 to-teal-500/20 border-[#137fec]/40"}`}>
      <div className="flex items-center gap-2 mb-4">
        <div className="p-2 rounded-lg bg-primary/20">
          <Sparkles className="w-5 h-5 text-primary" />
        </div>
        <h3 className={`text-lg font-semibold ${isDarkMode ? "text-white" : "text-slate-900"}`}>AI Analysis</h3>
      </div>
      ...
      >
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className={`text-sm font-medium ${isDarkMode ? "text-white" : "text-slate-800"}`}>Quick Tip</span>
        </div>
